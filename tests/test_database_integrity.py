...
class TestDatabaseIntegrity:
    ...
    def test_category_flags_are_mutually_exclusive(self, app):
    ...
    def test_body_mass_category_configuration(self, app):
        """Test that Body Mass category is properly configured"""
        with app.app_context():
            body_mass_categories = [cat for cat in services.get_all_categories() if cat.is_body_mass]
    ...
    def test_body_weight_exercise_category_configuration(self, app):
        """Test that body weight exercise categories are properly configured"""
        with app.app_context():
            body_weight_categories = [cat for cat in services.get_all_categories() if hasattr(cat, 'is_body_weight_exercise') and cat.is_body_weight_exercise]
    ...
    def test_database_schema_completeness(self, app):
        """Test that database schema has all required columns"""
        with app.app_context():
            # Test that critical columns exist
            sample_category = services.get_all_categories()[0]
            
            # Check for required attributes
            required_attributes = ['id', 'name', 'is_body_mass', 'is_body_weight_exercise', 'created_at']
            ...
            if entries:
                ...
                entry_attributes = ['id', 'weight', 'unit', 'category_id', 'created_at']
...
class TestProductionParityValidation:
    ...
    def test_body_mass_entry_saves_actual_weight_not_zero(self, app, sample_categories):
        ...
        with app.app_context():
            ...
            for weight in test_weights:
                ...
                entry = services.save_weight_entry(weight, 'kg', sample_categories['body_mass'].id, None)
    ...
    def test_form_submission_simulation_body_mass(self, app, client, sample_categories):
        ...
        with app.app_context():
            ...
            entries_before = len(services.get_all_entries())
            ...
            entries_after = services.get_all_entries()
    ...
    def test_category_identification_logic(self, app, sample_categories):
...
class TestMigrationRobustness:
    ...
    def test_migration_preserves_data_integrity(self, app):
        """Test that database migrations preserve existing data"""
        with app.app_context():
            # Get current categories and entries
            categories_before = services.get_all_categories()
            entries_before = services.get_all_entries()
            ...
            categories_after = services.get_all_categories()
            entries_after = services.get_all_entries()
    ...
    def test_production_data_characteristics(self, app):
